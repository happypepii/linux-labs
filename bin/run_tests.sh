...
base_cache_dir="${XDG_CACHE_HOME:-$HOME/.cache}/mff_nswi177_2026"
tests_repository="https://gitlab.mff.cuni.cz/teaching/nswi177/2026/common/tests.git"
...
while [ $# -gt 0 ]; do
    case "$1" in
        --help)
            echo "$0 [OPTS] TEST_PATTERN - NSWI177 local test runner wrapper"
            echo " --update     Force update of test repository"
            echo "              (automatic after $(( next_update_interval_sec / 60 )) minutes)"
            echo " --no-update  Prevent test repository update"
            echo " --verbose    Run in verbose mode"
            echo " TEST_PATTERN Which tests to execute (e.g. 01/dayname)"
            exit 0
            ;;
        --update)
            force_update=true
